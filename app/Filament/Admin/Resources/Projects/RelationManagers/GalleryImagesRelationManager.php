    ...
    public function form(Schema $schema): Schema
    {
        return $schema->components([
            FileUpload::make('path')
                ...
                ->dehydrateStateUsing(fn (?string $state, ?ProjectImage $record): ?string => $state ?? $record?->path)
                ->columnSpanFull(),
        ]);
    }
        ...
        return $table
            ...
            ->columns([
                ImageColumn::make('thumb')
                    ->label('Preview')
                    ->getStateUsing(fn (ProjectImage $record): string => Project::absoluteMediaUrl($record->path))
                    ->height(56)
